...
hdelta=0
#General info
Nn=c(10000)#sample sizec(500,1000,2000,2500,4000,5000)
K=100 #replications
g=1 #gamma
d=0 #delta
Bb=c(0.5)#beta
avec=c(0.2,0.35,0.5,0.8,1,1.2,1.5,1.8,2)#alpha
avec
  ...
  for(n in 1:length(Nn)){
    ...
    for(l in 1:length(avec)){#for each alpha at fixed beta=0,1
      for (j in 1:K){ #replicates
        ...
        ptm<-proc.time()
        u=GenSA(c(0.03,0.09),AV,lower=c(10^(-10),10^(-10)),upper=c(1,1),control = list(max.time=60, temperature=15))$par
        r=0
        v=0
        k=0
        s=0
...
E1=E[E[,1]==0.95,]
E05=E[E[,1]==0.5,]
CuF1_d0<-tapply(E0[,4],list(E0[,2],E0[,9]),rms)
CuF1_g0<-tapply(E0[,3],list(E0[,2],E0[,9]),rms)
CuF1_d1<-tapply(E1[,4],list(E1[,2],E1[,9]),rms)
CuF1_g1<-tapply(E1[,3],list(E1[,2],E1[,9]),rms)
CuF1_d05<-tapply(E05[,4],list(E05[,2],E05[,9]),rms)
CuF1_g05<-tapply(E05[,3],list(E05[,2],E05[,9]),rms)

op <- par(mfcol = c(1,2),mar=c(4,4,1,1),cex.sub=1.2,mgp=c(2.5,0.7,0),oma = c(2, 1, 1, 1))
...
CuF1_a0_u1<-tapply(E0[,28],list(E0[,2],E0[,9]),rms)
CuF1_a0_u2<-tapply(E0[,29],list(E0[,2],E0[,9]),rms)
#persp(CuF2_a0)
ChF_a0
ML_a0
QB_a0
...
CuF1_a1_u1<-tapply(E1[,28],list(E1[,2],E0[,9]),rms)
CuF1_a1_u2<-tapply(E1[,29],list(E1[,2],E0[,9]),rms)
CuF2_a1
ChF_a1
ML_a1
QB_a1
...
CuF1_b0_1<-tapply(E0[,30],list(E0[,2],E0[,9]),rms)
CuF1_b0_2<-tapply(E0[,31],list(E0[,2],E0[,9]),rms)
CuF2_b0
ChF_b0
ML_b0
QB_b0
